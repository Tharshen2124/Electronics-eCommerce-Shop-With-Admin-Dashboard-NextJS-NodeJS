...
test.describe('Filter and Sort Products', () => {
    ...
    test('Alternate Flow 1 - filter by availability status', async ({ page }) => {
        ...
        await page.goto('http://localhost:3000/shop?outOfStock=true&inStock=false&rating=0&price=3000&sort=defaultSort&page=1');
        await page.getByRole('link', { name: 'View product' }).first().click();
        
        await expect(page.getByText('Out of stock')).toBeVisible({ timeout: 10000 });
        
        await page.goto('http://localhost:3000/shop?outOfStock=true&inStock=true&rating=0&price=3000&sort=defaultSort&page=1');
